...
def load_data(root_dir='./',dataset_name='FTIR',scale_dataset=False,shuffle=-1,balance=0,excluded_class=None,window=1,return_scaler=False,saved_scaler=False, perc_split=[100,0,0]):
    ...
    if not os.path.isfile(dataset+'.scaler') and saved_scaler:
        ...
        saved_scaler=False
    ######
    train_set, test_set, _, mapping, scaler =fromMAT(path=dataset,varname=varname,perc_split=perc_split, seed=shuffle,scale_dataset=(scale_dataset and not saved_scaler),excluded_class=excluded_class,balance=100,window=window,augmentation_type='n')
    ######
